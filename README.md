...
摸底结束后，技能会先与学习者确认两件事，再开始教学：

- **学习终点**：学到哪里就停止，用"能独立做到 X"的形式描述，而不是一个笼统的主题；
- **学习深度**：三个颗粒度三选一，技能会基于摸底结果推荐一个：
  1. **概览**——用自己的话说清核心概念和适用场景（验证标准：独立复述）；
  2. **应用**——解释机制并解决标准及近迁移问题（验证标准：独立复述 + 近迁移）；
  3. **精通**——处理边界、例外和全新情境，或产出真实作品（验证标准：远迁移或真实产出）。

确认之后，技能会给出一条由 2-6 个检查点组成的学习路径和大致时间预估。每通过一个检查点，会用一行话标记进度（例如"检查点 2/4 已完成，按当前速度约剩 15 分钟"），让学习者始终知道自己学到了多少。达到约定终点后即进入巩固阶段，不会擅自扩大范围；想学更深需要明确选择。
...
- `练习模式`
- `回顾`
- `进度` / `progress`（查看检查点完成情况和剩余时间预估）
- `改目标` / `学更深`（重新确认终点或深度并调整路径）
- `跳过`
- `结束`
